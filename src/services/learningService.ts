...
export async function fetchArticles(category?: string): Promise<LearningArticle[]> {
  try {
    let query = supabase.from('learning_articles').select('*').eq('is_published', true).order('created_at', { ascending: false });
    ...
    if (!error && data && data.length > 0) {
      return data.map(a => ({
        id: a.id, category: a.category || '', title: a.title || '', summary: a.summary || '',
        content: a.content || '', reading_time: a.reading_time || '5 min', cover_image: a.cover_image || '',
...
export async function fetchArticleById(id: string): Promise<LearningArticle | undefined> {
  try {
    const { data, error } = await supabase.from('learning_articles').select('*').eq('id', id).single();
    if (!error && data) {
      return { id: data.id, category: data.category || '', title: data.title || '', summary: data.summary || '',
        content: data.content || '', reading_time: data.reading_time || '5 min', cover_image: data.cover_image || '',
